...
import useTopScroll from "@/client/hooks/useTopScroll";
...
const ProductsListing = ({ categorySlug }: ProductsListingProps) => {
    const [pagination, setPagination] = useState<PaginationRequest>({
        page: 0,
        size: 12,
    });
    useTopScroll(0, [pagination.page]);
    ...
    return (
        <Container>
            <section className="w-full py-8 md:py-10">
                <SectionHeader title={categorySlug ? "Category Products" : "Products"} />
                    ...
                    <>
                        ...
                        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t pt-6 sm:flex-row">
                            <p className="text-sm text-muted-foreground">
                                ...
                                <span className="font-medium text-foreground">{productPage?.totalPages ?? 0}</span>
                            </p>
